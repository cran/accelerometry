...
NumericMatrix personvars(NumericMatrix dayvars, int rows, int days, int wk, int we) {
  ...
  if (dayvars(0,2)==1) {
    ...
    if (dayvars(0,1)==1 || dayvars(0,1)==7) {
      out(0,3) += 1;
      for (int a = 3; a < 66; ++a)
        out(0,a+128) += dayvars(0,a);
    }
    else {
      out(0,2) += 1;
      for (int a = 3; a < 66; ++a)
        out(0,a+65) += dayvars(0,a);
    }
  }
  for (int b = 1; b < n; ++b) {
    if (dayvars(b,0)!=dayvars(b-1,0)) {
      ...
      for (int a = 68; a < 131; ++a)
        out(index,a) = out(index,a)/out(index,2);
      for (int a = 131; a < 194; ++a)
    ...
    if (dayvars(b,2)==1) {
      ...
      if (dayvars(b,1)==1 || dayvars(b,1)==7) {
        out(index,3) += 1;
        for (int a = 3; a < 66; ++a)
          out(index,a+128) += dayvars(b,a);
      }
      else {
        out(index,2) += 1;
        for (int a = 3; a < 66; ++a)
          out(index,a+65) += dayvars(b,a);
      }
    }
  }
  if (out(index,1)>=days && out(index,2)>=wk && out(index,3)>=we) out(index,4) = 1;
